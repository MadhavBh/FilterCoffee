...
build/kernel.bin: build/boot.o build/mbhead.o build/vga.o build/kernel.o build/isr.o build/idt.o build/gdt.o build/interrupt.o build/descriptor.o build/ports.o build/keyboard.o linker.ld
	#ld -n -m elf_i386 -o build/kernel.bin -T linker.ld build/boot.o build/mbhead.o
	/home/madhavbh/opt/cross/bin/i686-elf-gcc -T src/linker.ld -o build/kernel.bin -ffreestanding -O2 -nostdlib build/mbhead.o build/boot.o build/vga.o build/isr.o build/idt.o build/gdt.o build/interrupt.o build/descriptor.o build/ports.o build/keyboard.o  build/kernel.o -lgcc
